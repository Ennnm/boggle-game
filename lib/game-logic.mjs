    ...
    if (cellChar === wordChar || cellChar === '*') {
      ...
      nextCoords.forEach((coord) => {
        if (!(coord in visited)) {
          ...
          const searchResult = dfs([coord[0], coord[1]], remaining, [
            ...visited,
          ]);
          console.log('searchResult :>> ', searchResult);
          console.log('visited before del :>> ', visited);
          visited.pop();
          console.log('visited after del :>> ', visited);
          if (searchResult) isFound = true;
        }
      });
      console.log('after matching char');
      // need backtracking to undo visited
      // delete visited[at];
    } else {
